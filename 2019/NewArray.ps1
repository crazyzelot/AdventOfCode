...
$yIsTrue = 0
$zIsTrue = 0
$PreviousIoPositions = "-8,-10,0"
$PreviousEuropaPositions = "5,5,10"
$PreviousGanymedePositions = "2,-7,3"
$PreviousCallistoPositions = "9,-8,-3"
$IoInitial = $PreviousIoPositions.Split(",")
$EuropaInitial = $PreviousEuropaPositions.Split(",")
$GanymedeInitial = $PreviousGanymedePositions.Split(",")
$CallistoInitial  = $PreviousCallistoPositions.Split(",")
...
while($isTrue -eq 0)
{
    ...
    {
        $output = $time/1000000
        Write-Host "`r`nRun $output million runs."
    }
    if($IoInitial[0] -eq $IoPos[0] -and $EuropaInitial[0] -eq $EuropaPos[0] -and $GanymedeInitial[0] -eq $GanymedePos[0] -and $CallistoInitial[0] -eq $CallistoPos[0] -and $xIsTrue -eq 0)
    {
        $xIsTrue = 1
        $xTime = $time
        Write-Host Period of x is $xTime
        $PreviousIoPositions
        $PreviousEuropaPositions
        $PreviousGanymedePositions
        $PreviousCallistoPositions
    }
    if($IoInitial[1] -eq $IoPos[1] -and $EuropaInitial[1] -eq $EuropaPos[1] -and $GanymedeInitial[1] -eq $GanymedePos[1] -and $CallistoInitial[1] -eq $CallistoPos[1] -and $yIsTrue -eq 0)
    {
        $yIsTrue = 1
        $yTime = $time
        Write-Host Period of y is $yTime
        $PreviousIoPositions
            $PreviousEuropaPositions
            $PreviousGanymedePositions
            $PreviousCallistoPositions
    }
    if($IoInitial[2] -eq $IoPos[2] -and $EuropaInitial[2] -eq $EuropaPos[2] -and $GanymedeInitial[2] -eq $GanymedePos[2] -and $CallistoInitial[2] -eq $CallistoPos[2] -and $zIsTrue -eq 0)
    {
        $zIsTrue = 1
        $zTime = $time
        Write-Host Period of z is $zTime
            $PreviousIoPositions
            $PreviousEuropaPositions
            $PreviousGanymedePositions
            $PreviousCallistoPositions
    }
    #if($xIsTrue -eq 1 -and $yIsTrue -eq 1 -and $zIsTrue -eq 1)
    #{
    #    $isTrue = 1
    #    $output = $xTime * $yTime * $zTime
    #    Write-Host $Actual Answer is $output
    #}
