...
while ($count -lt $pointsOnWire1.Length)
{
    Write-Host "Entering Loop 1"
    $count2 = 0
    while ($count2 -lt $pointsOnWire2.Length)
    {
        Write-Host "Entering Loop 2"
        if ($pointsOnWire1[$count] -eq $pointsOnWire2[$count2])
        {
            #$pointsOnWire1[$count]
            $points = $pointsOnWire1[$count].Split(" ")
